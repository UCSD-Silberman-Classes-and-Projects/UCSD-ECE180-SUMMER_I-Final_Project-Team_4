...
def run(model_path, front_port, rear_port, log_path, stub=False,
        no_drive=False, telemetry_file=None):
    camera = Camera()
    ...
    try:
        while True:
            ...
            # --- telemetry (opt-in; laptop/radar_view.py's live map) ---
            # Side-effect only -- writes a JSON snapshot for a laptop-side
            # viewer to poll over SSH. Off by default (telemetry_file is
            # None unless --telemetry-file is passed) and never raises, so
            # it cannot affect the control loop above or the caught/logging
            # below even if the write itself fails.
            if telemetry_file is not None:
                telemetry.write_frame(telemetry_file, points, bearing_ccw,
                                      pursuer_range, heading, speed,
                                      proximity)
...
def main():
    ...
    ap.add_argument("--telemetry-file", default=None,
                    help="write a JSON snapshot here each tick for "
                         "laptop/radar_view.py to poll live (off by "
                         "default; e.g. /tmp/mousebot_telemetry.json)")
    args = ap.parse_args()
